...
flower_info = {
    ...
    "tulip": {
        "name": "Tulip",
        "description": "Spring-blooming bulbs with brightly colored cup-shaped flowers.",
        "diseases": [
            {"name": "Botrytis Blight", "cause": "Botrytis tulipae", "symptoms": "Brown spots on leaves and flowers", "management": "Destroy infected plants, rotate crops"},
            {"name": "Tulip Fire", "cause": "Botrytis tulipae", "symptoms": "Twisted and spotted leaves", "management": "Avoid overhead watering, destroy infected material"}
        ]
    },
    "aster": {
        "name": "Aster",
        "description": "Daisy-like perennials with star-shaped flower heads.",
        "diseases": [
            {"name": "Powdery Mildew", "cause": "Fungal spores", "symptoms": "White powder on foliage", "management": "Use resistant varieties and apply fungicides"},
            {"name": "Aster Yellows", "cause": "Phytoplasma", "symptoms": "Stunted growth and yellowing leaves", "management": "Control insects, remove infected plants"}
        ]
    },
    "tuberose": {
        "name": "Tuberose",
        "description": "Fragrant white flower spikes often used in perfumes.",
        "diseases": [
            {"name": "Stem Rot", "cause": "Sclerotium rolfsii", "symptoms": "Stem base rots and plant collapses", "management": "Improve drainage and use fungicides"},
            {"name": "Leaf Blight", "cause": "Alternaria spp.", "symptoms": "Brown spots on leaves", "management": "Remove infected leaves and spray fungicide"}
        ]
    },
    "daffodil": {
        "name": "Daffodil",
        "description": "Spring-flowering bulbs with trumpet-shaped flowers.",
        "diseases": [
            {"name": "Basal Rot", "cause": "Fusarium oxysporum", "symptoms": "Bulb rot and poor flowering", "management": "Avoid injury to bulbs, use clean tools"},
            {"name": "Narcissus Smoulder", "cause": "Botrytis narcissicola", "symptoms": "Gray mold and flower rot", "management": "Remove infected parts, apply fungicide"}
        ]
    }
}

flower_names = list(flower_info.keys())
...
def flower_details(flower_name):
    ...
    flower_data = flower_info.get(flower_key)

    if flower_data:
        flower_index = flower_names.index(flower_key)
        next_flower = flower_names[(flower_index + 1) % len(flower_names)]
        prev_flower = flower_names[(flower_index - 1) % len(flower_names)]

        return render_template('flower_details.html', flower_data=flower_data,
                               next_flower=next_flower, prev_flower=prev_flower, active_page=None)
    else:
